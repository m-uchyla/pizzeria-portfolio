                ...
                <a href='/'>
                  <div className='flex flex-row items-center'>
                    <p className='sm:text-[20px] text-[12px] text-white font-semibold mr-1'>Pizza</p>
                      <GiFullPizza className='sm:text-[40px] text-[25px] text-orange-500'/> 
                    <p className='sm:text-[20px] text-[12px] text-white font-semibold ml-1'>Eccezionale</p>
                  </div>
                </a>
                <ul className='flex items-center'>
                  {privacy.map((item, index) => (
                    <li key={item.id} className={`links-footer sm:text-[16px] text-[7px] ${index === privacy.length - 1 ? "mr-0" : "mr-5"} cursor-pointer`}>{item.title}</li>
                  ))}
                </ul>
              </div>
            </div>
